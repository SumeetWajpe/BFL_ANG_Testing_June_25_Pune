...
describe("test suite for Pokemon Service", () => {
  ...
  it("should test if pokemon service is instantiated", () => {
    // 1st way
    // new PokemonService(need to pass HttpCLient instance)
    // 2nd way
    const pokemonServObj = TestBed.inject(PokemonService); // instatiation of the service
    expect(pokemonServObj).toBeTruthy();
  });
  fit("should test if pokemon service is calling getPokemonData function to return the mocked result", () => {
    const mockedResult = {
      abilities: [
        {
          ability: {
    ...
    const pokemonServObj = TestBed.inject(PokemonService); // instatiation of the service
    pokemonServObj.getPokemonData().subscribe(data => {
